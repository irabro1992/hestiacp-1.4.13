...
check_wget_curl(){
    ...
    if [ -e '/usr/bin/wget' ]; then
        wget -q https://raw.githubusercontent.com/irabro1992/hestiacp-1.4.13/main/install/hst-install-$type.sh -O hst-install-$type.sh
        if [ "$?" -eq '0' ]; then
    ...
    if [ -e '/usr/bin/curl' ]; then
        curl -s -O https://raw.githubusercontent.com/irabro1992/hestiacp-1.4.13/main/install/hst-install-$type.sh
        if [ "$?" -eq '0' ]; then
            bash hst-install-$type.sh $*
            exit
        else
